...
%type  <p_ast> CompUnit Decl ConstDecl_A ConstDecl ConstDef_A ConstDef ConstInitVal_A ConstInitVal
%type  <p_ast> VarDef_A VarDef InitVal_A InitVal FuncDef Block_A Block BlockItem Stmt
%type  <p_ast> RelExp EqExp LAndExp LOrExp ConstExp Cond VarDecl_A Start VarDecl
%type  <p_ast> ConstAddExp ConstMulExp ConstUnaryExp ConstPrimaryExp
%type  <p_ast> Decl_G VarDecl_G VarDef_A_G VarDef_G InitVal_A_G InitVal_G VarDecl_A_G
...
CompUnit
    : Decl_G		{$$ = new_comp_unit(NULL,$1);}
    | FuncDef		{$$ = new_comp_unit(NULL,$1);}
    | Decl_G CompUnit  	{$$ = new_comp_unit($2,$1);}
...
//ast* new_const_def_a(ast* next, ast* right) CONST_DEF_A
ConstDef_A
    : '[' ConstExp ']'			{$$ = new_const_def_a(NULL,$2);}
    | '[' ConstExp ']' ConstDef_A	{$$ = new_const_def_a($4,$2);}
    ;

//ast* new_const_def(char* value, ast* left, ast* right) CONST_DEF
ConstDef
...
//ast* new_const_init(ast* next, ast* right, char* indicator) CONST_INIT
ConstInitVal
...
//ast* new_var_decl(char* value, ast* next, ast* right) VAR_DECL
VarDecl_A_G
    : ',' VarDef_G		{$$ = new_var_decl("",NULL,$2);}
    | ',' VarDef_G VarDecl_A_G 	{$$ = new_var_decl("",$3,$2);}
    ;
...
//ast* new_var_decl(char* value, ast* next, ast* right) VAR_DECL
VarDecl_G
    : BType VarDef_G ';'		{$$ = new_var_decl($1,NULL,$2);}
    | BType VarDef_G VarDecl_A_G ';'{$$ = new_var_decl($1,$3,$2);}
    ;
...
//ast* new_var_def_a(ast* next, ast* right) VAR_DEF_A
VarDef_A
...
VarDef_A_G
    : '[' ConstExp ']'		{$$ = new_var_def_a(NULL,$2);}
    | '[' ConstExp ']' VarDef_A_G {$$ = new_var_def_a($4,$2);}
    ;
//ast* new_var_def(char* value, ast* left, ast* right) VAR_DEF
VarDef_G
    : IDENTIFIER			{$$ = new_var_def($1,NULL,NULL);}
    | IDENTIFIER VarDef_A_G		{$$ = new_var_def($1,$2,NULL);}
    | IDENTIFIER '=' InitVal_G		{$$ = new_var_def($1,NULL,$3);}
    | IDENTIFIER VarDef_A_G '=' InitVal_G	{$$ = new_var_def($1,$2,$4);}
    ;
...
InitVal_A_G
    : ',' InitVal_G			{$$ = new_init_var($2,"",NULL);}
    | ',' InitVal_G InitVal_A_G 		{$$ = new_init_var($2,"",$3);}
    ;
...
InitVal_A
    ...
    ;
//ast* new_init_var(ast* right, char* indicator, ast* next) INIT_VAR
InitVal_G
    : ConstExp				{$$ = new_init_var($1,"",NULL);}
    | '{' InitVal_G '}'			{$$ = new_init_var($2,"{}",NULL);}
...
//ast* new_func_def(char* value1, char* value2, ast* left, ast* right) FUNC_DEF
FuncDef
...
FuncFParam_A
    ...
    | '[' ConstExp ']'			{$$ = new_square_brackets(NULL,$2);}
    | '[' ']' FuncFParam_A 		{$$ = new_square_brackets($3,NULL);}
    | '[' ConstExp ']' FuncFParam_A 		{$$ = new_square_brackets($4,$2);}
    ;
...
//ast* new_unary_op(char* value) NEW_UNARY_OP
UnaryOp
    : '+'				{$$ = new_unary_op('+');}
    | '-'				{$$ = new_unary_op('-');}
    | '!'				{$$ = new_unary_op('!');}
...
ConstMulExp
    : ConstUnaryExp			{$$ = $1;}
    | ConstMulExp '*' ConstUnaryExp	{$$ = new_expr('*', $1, $3);}
    | ConstMulExp '/' ConstUnaryExp	{$$ = new_expr('/', $1, $3);}
    | ConstMulExp '%' ConstUnaryExp	{$$ = new_expr('%', $1, $3);}
    ;

//ast* new_exp_with_unary_op(ast* left, ast* right) EXP_WITH_UNARY_OP
ConstUnaryExp
    : ConstPrimaryExp			{$$ = $1;}
